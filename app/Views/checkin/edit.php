        ...
        <div class="card-body">
            <form action="<?= base_url('/checkin/update/' . $checkin['ID_CHECKIN']) ?>" method="post">
                <div class="form-group">
                    ...
                    <select name="id_tiket" id="id_tiket" class="form-control" style="background:#f8fafc; pointer-events:none;" readonly>
                        <?php foreach ($tikets as $t): ?>
                            <option value="<?= $t['ID_TIKET'] ?>" <?= $t['ID_TIKET'] == $checkin['ID_TIKET'] ? 'selected' : '' ?>>
                                <?= esc($t['NOMER_TIKET']) ?> - <?= esc($t['NAMA_PENUMPANG']) ?> | Kelas: <?= esc($t['KELAS_TIKET'] ?? '-') ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                </div>
...
<script>
    ...
    // Load available seats on page load for the selected ticket
    document.addEventListener("DOMContentLoaded", function() {
        const idTiket = document.getElementById('id_tiket').value;
        const currentSeatId = <?= $checkin['ID_KURSI'] ?>;
        ...
        fetch('<?= base_url('/checkin/get-available-seats/') ?>' + idTiket)
            ...
            .then(data => {
                ...
                // Remove the current seat from occupiedIds so we can visually select/unselect it
                const index = occupiedIds.indexOf(currentSeatId);
                if (index > -1) {
                    occupiedIds.splice(index, 1);
                }
                ...
                // Populate regular dropdown option
                seats.forEach(seat => {
                    ...
                    if (!isOccupied && classMatch) {
                        const selectedAttr = (seat.ID_KURSI == currentSeatId) ? 'selected' : '';
                        seatSelect.innerHTML += `<option value="${seat.ID_KURSI}" ${selectedAttr}>${seat.NO_KURSI2} (${seat.KELAS_PENERBANAN})</option>`;
                    }
                });
                ...
                if (rows.length === 0) {
                    ...
                    rows.forEach(r => {
                        ...
                        leftCols.forEach(c => {
                            ...
                            const seatEl = createSeatElement(seat, c, occupiedIds, classColors, ticketClass, currentSeatId);
                ...
                // Trigger initial selection text
                seatSelect.onchange();
    ...
    function createSeatElement(seat, colLetter, occupiedIds, classColors, ticketClass, currentSeatId) {
        if (!seat) {
        ...
        const isOccupied = occupiedIds.includes(parseInt(seat.ID_KURSI));
        const classMatch = !ticketClass || seat.KELAS_PENERBANAN === ticketClass;
        const isCurrent = seat.ID_KURSI == currentSeatId;
